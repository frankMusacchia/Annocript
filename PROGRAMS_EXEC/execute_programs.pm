...
use USEFUL::utilities qw(delete_file delete_directory exec_command 
                    count_sequences check_presence extract_name extract_special_chars create_slices);

#EXTRACT PARAMETERS FROM THE CONFIG FILE
my $configHash;

#An array contains the arguments to be passed. If something that you need is not here please add	
my @blastArgs = ('word_size','evalue','num_threads','num_descriptions','num_alignments','max_target_seqs','outfmt','threshold');
...
sub execute_programs {
  ...
  #Variables for computing execution times
    ...
    if ($configHash->{'sequencesAlphabet'} eq 'dna'){
      ...
      print "Finished $blastToUse against SwissProt DB in ".$partDuration." seconds\n\n";
      $partTime = time;
      ...
      $partDuration = time - $partTime;
      print "Finished $blastToUse against Trembl DB in ".$partDuration." seconds\n\n";
      $partTime = time;
    }
    if ( $configHash->{'dbInUse'} eq 'uniref' ){
      ...
      $partDuration = time - $partTime;
      print "Finished $blastToUse against Uniref DB in ".$partDuration." seconds\n\n";
      $partTime = time;
    }
  }
  ...
  #Do rps blast
    ...
    blast_parallel_run($fastaSeqs, $blastToUsePath, $cdDBPath, $threads, $rpstBlastnOutPath);
		...
		#&run_blast($fastaSeqs, $blastToUsePath, $cdDBPath, $threads, $rpstBlastnOutPath);
		#Check the output if is good
    #$rpstBlastnOutPath = $rpstBlastnOutPath.'1';
		#if (&checkOutput($rpstBlastnOutPath,$totSequences,"blast") == 0){
  ...
  if ($configHash->{'doBlastn'} eq "YES"){
    ...
    print "\n(".scalar(localtime).") Running $blastToUse against RFAM\n";#LOG
    my $blastnOutPath = 'blastn/'.$configHash->{'blastnOut'};
    &run_blast($fastaSeqs,$blastToUsePath,$configHash->{'rfamDB'}, $threads,$blastnOutPath);
    #Check the output if is good
    ...
    $partDuration = time - $partTime;
    print "Finished $blastToUse against RFAM db in ".$partDuration." seconds\n\n";
    $partTime = time;
  }
  if ($configHash->{'sequencesAlphabet'} eq 'dna') {
	...
	print "\n".scalar(localtime).": Annocript has finished the execution of programs in: ".$globalDuration." seconds\n\n";#LOG
}
...
sub run_blast_indexing {
  ...
  if($program eq "blastx"){
   ...
   ( system ($command)  ) == 0
    ...
    my $outFile1=$file.".nhr";
		my $outFile2=$file.".nin";
		my $outFile3=$file.".nsq";
		if((!(-e "$outFile1")) or (!(-e "$outFile2")) or (!(-e "$outFile3")) ){
		 die ("\nERROR [$?]: $program run not successful: ?\n");
		}
	}
...
=head2 check_makeblastdb_out
 ...
 Function: 	Check if the indexing of a database has been good. In particular is used the function blastdbcmd
...
sub portrait_split_and_run {
  ...
  if(!-e "$file"){
    ...
    if(-z "$file"){
      ...
      #my $mergedOut = $workingDir."/".$outFolder."/".$outRealName;#merged_out.txt";  
      ...
      my $toExam = $configHash->{'split_num'};#100;# sequences per time to analyze
...
sub portrait_exec_blocks {
  ...
  while( (my $row = <IN>)  ) {
    if ($stopPortrait == 0){
      ...
      if($row =~ /^\>/){
        $stopSeq++;
        #print "Elaborating sequence: $stopSeq\n";#DEBUGCODE
        ...
        if (($seqCount ==  $toExam) and ($blockNum < (int ( $totSequences/$toExam)) ) ){
         # print "sequenza: $stopSeq blocchi da:$toExam blocco num:$blockNum non superare:".int($totSequences/$toExam)."\n";#DEBUGCODE
          ...
          while ( $done == 0){
            ...
            if( (-e "$outFile") ){
              #...and the number of results is correct
              if ( $resultNum == $toExam ){
                ...
                print "Good! Number of elements in $blockNum output :".`fgrep -c '>' $outFile`."\n";#DEBUG CODE
              }else{   
                $badRuns++;
                if ($badRuns >= $maxBadRuns){
                  ...
                  print ("\n[>>> WARNING: PORTRAIT block $blockNum run not successful! Output differs from $toExam. \nRestarting ($badRuns trial)....<<<]\n"); 
                }
              }
            }else{
              $badRuns++;
            ...
            #( system ('rm sequence.fasta_*')  ) == 0
                #or print "ERROR in execute_programs::portrait_exec_blocks: an error occurred while removing  sequence.fasta_*: $?";
          }
...
sub run_d2p {
  ...
  ( system ("$programPath -O tab -o $mode -r all $cleanFile > $outFile")  ) == 0
...
sub portrait_cleanFasta{
  ...
  #Erase all the longest than 100 thousands sequences
  #or portrait will give "Illegal division by zero"
  print "PORTRAIT CLEAN: Checked the sequences! (Longest than 100 thousands are not accepted)\n";
  ...
  print "PORTRAIT CLEAN: Finished adapting your query file for PORTRAIT!\n";
    ...
    if ($Ns < ( ($seq->length/100) * $maxPercNs) ){
      if ($basecount > $minPortraitSeqLength) {
       if ($basecount < $maxPortraitSeqLength){
          ...
          $countLongest++;
          print "Sequence $display erased because it's longer ($basecount) than the maximum ($maxPortraitSeqLength)\n";
       }
      }else{
          $countShort++;
          print "Sequence $display erased because it's shorter ($basecount) than the minimum ($minPortraitSeqLength)\n";
       }
    }else{
      $countNs++;
      print "Sequence $display erased because it has $Ns Ns and it's length is $basecount (too much Ns!!!)\n";
    }
	}
...
=head2 parallel_execution
 ...
 Function: for each FASTA file present in the fastaFolder executes once the command cmd and creates a different output
          which name is given by out and a progressive number.
