#############################gff3_manager.pm#################
# This script uses the information taken by the programs (BLAST, PORTRAIT, DNA2PEP) to
...
package GFF3_AND_OUTPUT::gff3_manager;
use Exporter();

@ISA = qw(Exporter);
@EXPORT_OK = qw(execute_gff_manager);
...
use Switch;#To use switch statement
use Storable;
...
#Annocript libraries
use GFF3_AND_OUTPUT::output_and_stats	qw( lncRNA_coding_fasta_file_generator lncRNA_fasta_file_generator extractAnnotated extractStatistics settingsForUniprotKB settingsForUniref);
use USEFUL::utilities qw( extract_name print_array try_exec_command);
...
#This is the header of the output table. Here you can add fields if you need
#IMPORTANT: BEFORE TO ADD LOOK AT THE DOCUMENTATION OF THE SOFTWARE!!!!
my @finalTabHeaderUNIPROT = qw(TranscriptName
                            TransLength
                            Count
                            HSPNameSP
                            HSPLengthSP
...
my @finalTabHeaderUNIREF = qw(TranscriptName
                            TransLength
                            Count
                            HSPNameSP
                            HSPLengthSP
...
sub checkOutputsPresence{
    ...
    my $hashVarName = $outputName.'Present';
    #Check the presence of the blastx output
    if (-e $outFolder.'/'.$outputName){
      $configHash->{$hashVarName} = 'YES';
    }else{$configHash->{$hashVarName} = 'NO';}
  }


  
=head2 execute_gff_manager

 Title : execute_gff_manager
 Usage : execute_gff_manager( -configHash => 'configHash',
					);
...
sub execute_gff_manager{
  ...
  if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){  
    $finalTabHeader = join("\t",@finalTabHeaderUNIPROT);
    pop(@finalTabHeaderUNIPROT);
    $ridFinalTabHeader = join("\t",@finalTabHeaderUNIPROT);#The reduced file header is one with all info and not sequence
    $os_tax='os';
    $os_taxTable = 'OSType';
    ...
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxSPOut'},$configHash->{'blastxSPOutType'},$configHash->{'blastxSPGFF3FileName'},"blastx",1,$configHash->{'database'},'sp','OSType','os','uniprotkbViewSeq') > 0){
      ...
      $partDuration = time - $partTime;
      print "Finished GFF3 conversion of output with Swiss-Prot in $partDuration\n";
      $partTime = time;
     
      if ( $configHash->{'useGFFDB'} eq 'YES' ){
        gff3_2_db($configHash->{'blastxSPGFF3FileName'},$configHash->{'gffDB'},$configHash->{'mySqlUser'},$configHash->{'mySqlPass'});
        $partDuration = time - $partTime;
        print "Finished storing GFF3 for blastx output from Tremble in $partDuration\n";
        $partTime = time;
      }
    ...
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxTROut'},$configHash->{'blastxTROutType'},$configHash->{'blastxTRGFF3FileName'},"blastx",1,$configHash->{'database'},'tr','OSType','os','uniprotkbViewSeq') >0 ){
      ...
      $partDuration = time - $partTime;
      print "Finished GFF3 conversion of output with Tremble in $partDuration\n";
      $partTime = time;
      
      if ( $configHash->{'useGFFDB'} eq 'YES' ){
        $partDuration = time - $partTime;
        gff3_2_db($configHash->{'blastxTRGFF3FileName'},$configHash->{'gffDB'});
        print "Finished storing GFF3 for blastx output from Swiss-Prot in $partDuration\n";
        $partTime = time;  
      }
 ...
 if ( $configHash->{'dbInUse'} eq 'uniref' ){
   $finalTabHeader = join("\t",@finalTabHeaderUNIREF);
   pop(@finalTabHeaderUNIREF);
   $ridFinalTabHeader = join("\t",@finalTabHeaderUNIREF);#The reduced file header is one with all info and not sequence
   $os_tax='os';
    $os_taxTable = 'OSType';
    $view = 'unirefViewSeq';
   ...
   #Extracting information from the BLAST out for swiss-prot and creating GFF3 file						
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxUnirefOut'},$configHash->{'blastxUnirefOutType'},$configHash->{'blastxUnirefGFF3FileName'},"blastx",1,$configHash->{'database'},'uniref','taxType','tax','unirefViewSeq') > 0){
      $configHash->{'blastxUnirefOutPresent'} = 'YES';#A variable to know if this output has been created
      $partDuration = time - $partTime;
      print "Finished GFF3 conversion of output with Uniref in $partDuration\n";
      $partTime = time;
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE RPS BLAST OUTPUT<<<<<<<<<<<<<<\n";#LOG
    ...
    $configHash->{'rpstblastnGFF3FileName'},"rpstblastn",2,undef,'none',undef,undef,undef) > 0){
    $configHash->{'rpstblastnOutPresent'} = 'YES';#A variable to know if this output has been created
    $partDuration = time - $partTime;
    print "Finished GFF3 conversion of rpstblastn output in $partDuration seconds\n";
    $partTime = time;
    
    $partDuration = time - $partTime;
    gff3_2_db($configHash->{'rpstblastnGFF3FileName'},$configHash->{'gffDB'});
    print "Finished storing GFF3 for RPS BLAST output in $partDuration seconds\n";
    $partTime = time;
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE BLASTN (TBLASTN) OUTPUT<<<<<<<<<<<<<<\n";#LOG
    ...
    $partDuration = time - $partTime;
    print "Finished GFF3 conversion of blastn output in $partDuration seconds\n";
    $partTime = time;
    ...
    if ( $configHash->{'useGFFDB'} eq 'YES' ){
      ...
      $partDuration = time - $partTime;
      print "Finished storing GFF3 for BLASTN (TBLASTN) output in ".$partDuration." seconds\n";
      $partTime = time;
    }
   
  }else{ $configHash->{'blastnOutPresent'} = 'NO';}
  
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE PORTRAIT OUTPUT<<<<<<<<<<<<<<\n";#LOG
	#NON CODING 
	if ( portraitout_2_gff3AndHash($configHash->{'portraitOut'},$configHash->{'nonCodingAlgorithm'},$configHash->{'nonCodingGFF3FileName'}) > 0){
     $configHash->{'portraitOutPresent'} = 'YES';#A variable to know if this output has been created
    $partDuration = time - $partTime;
    print "Finished GFF3 conversion of non coding search algorithm output in $partDuration seconds\n";
    
    if ( $configHash->{'useGFFDB'} eq 'YES' ){
      $partTime = time;
      gff3_2_db($configHash->{'nonCodingGFF3FileName'},$configHash->{'gffDB'});
      $partDuration = time - $partTime;
      print "Finished storing of GFF3 for non coding search algorithm output in $partDuration seconds\n";
      $partTime = time;
    }
  }else{$configHash->{'portraitOutPresent'} = 'NO';}
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE DNA2PEP OUTPUT<<<<<<<<<<<<<<\n";#LOG
    ...
    $partDuration = time - $partTime;
    print "Finished GFF3 conversion of ORF search output in ".$partDuration." seconds\n";
    ...
    if ( $configHash->{'useGFFDB'} eq 'YES' ){
      ...
      $partDuration = time - $partTime;
      print "Finished storing of GFF3 for DNA2PEP output in ".$partDuration." seconds\n";
      $partTime = time;
    }
  }else{$configHash->{'dna2pepOutPresent'} = 'NO';}
  
  #print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE COUNT OUTPUT<<<<<<<<<<<<<<\n";#LOG
  #count
	if (count_2_gff3AndHash($configHash->{'outCount'},$configHash->{'countAlgorithm'},$configHash->{'countGFF3FileName'}) > 0){
    $configHash->{'countOutPresent'} = 'YES';#A variable to know if this output has been created
    $partDuration = time - $partTime;
    print "Finished GFF3 conversion of count output in ".$partDuration." seconds\n";
    
    if ( $configHash->{'useGFFDB'} eq 'YES' ){
      $partTime = time;
      gff3_2_db($configHash->{'countGFF3FileName'},$configHash->{'gffDB'});
      $partDuration = time - $partTime;
      
      print "Finished storing of GFF3 for count output in ".$partDuration." seconds\n";
      $partTime = time;
    }
  }else{$configHash->{'countOutPresent'} = 'NO';}
  ...
  printAllInfo($configHash->{'gffDB'},$configHash->{'outFileName'},$configHash->{'outFiltered'},$outHashFile);#$configHash->{'storeBlastx'},$outHashFile);
	$partDuration = time - $partTime;
	print "Finished printing all algorithms informations in ".$partDuration."\n";
	$partTime = time;
  
  #Check if the dna2pep and portrait outputs are present. If they are, then some other file will be created
  checkOutputsPresence('dna2pepOut');
  checkOutputsPresence('portraitOut');
  ...
  #Here the variables are set depending from the database used
  if ($configHash->{'dbInUse'} eq "uniprotkb"){
  ...
  #Only if the output of portrait is present then we can create a file with the non coding sequences
  #Look at lncRNA_fasta_file_generator subroutine to see what are the constrains to be non coding
  if ( $configHash->{'portraitOutPresent'} eq 'YES'){
  ...
  if ( $configHash->{'useGFFDB'} eq 'NO' ){
    ...
    drop_database($configHash->{'gffDB'});
  }
...
sub dna2pepout_2_gff3AndHash{
  ...
  if ( -e $dna2pepOutFile ){
    ...
    close(DNA2PEP_OUT);
  }else{ print("GFF FILE for dna2pep has not been created! Can't open $dna2pepOutFile\n");}
  close(GFF_FILE);
  return $return;
}
...
=head2 count_2_gff3AndHash

 Title : count_2_gff3AndHash
 Usage : count_2_gff3AndHash( -file => 'dna2pepout_2out',
					);

 Function: creates a gff3 file from the count of reads and an Hash containing all the data.
            The file has to be in a specific format and is the count of reads for each transcript.
 
 Returns : a gff3 file

=cut
sub count_2_gff3AndHash{
	my $countOutFile = shift;
	my $algorithmType = shift;
	my $gff3FileName = shift;

	$countOutFile = $outFolder."/".$countOutFile;
	$gff3FileName = $gffFolder."/".$gff3FileName;
  
  my $return = 0;#If it's 1 then the subroutine has been executed

  my $countCol = $configHash->{'countCol'};
  
  if ( -e $countOutFile ){
    $return = 1;
    open (COUNT_OUT,"<$countOutFile") || die ( "\nERROR [$?]: Cannot open count out file for read!!! The count file must have this".
                            "path: $countOutFile. And must have a specific format.".
                            " Read manual for this. Exiting\n"); 
    open (GFF_FILE,">$gff3FileName") || die ( "\nERROR [$?]: Cannot open gff file to write: ?" );
    print GFF_FILE "##gff-version 3 \n";
    ...
    while(<COUNT_OUT>){
        ...
        #Write the GFF3 file 
        print GFF_FILE $prt[0]."\t".$algorithmType."\t"."region\t";#WE CHOOSE REGION SINCE THEY ARE SEQUENCES
        print GFF_FILE "10\t20\t".$prt[4]."\t.\t.\t";#10-20 ARE INVENTED!!!!
        print GFF_FILE "ID=".$featId.";c=".$prt[ $countCol]."\n";
        $featId++;	
        $outHash->{$prt[0]}->{'count'} = $prt[ $countCol ]; 
      #}
      $rowCount++;
   }
   close(COUNT_OUT);
   close(GFF_FILE);
   print("GFF FILE for counts has been created!\n");
  }#else{print "Count of reads for each sequence will not be added. $countOutFile is not present.\n";}
...
sub portraitout_2_gff3AndHash{
  ...
  if (open (PORTRAIT_OUT,"<$portraitOutFile") ){
    open (GFF_FILE,">$gff3FileName") || die ( "\nERROR [$?]: Cannot open gff file to write: ?" );
    print GFF_FILE "##gff-version 3 \n";
  ...
  }else{ print("GFF FILE for coding/non-coding has not been created! Can't open $portraitOutFile\n");}
...
=head2 multiBlastout_2_gff3AndHash
 ...
 Function: creates a gff3 file and can use different blastx outputs taken from a folder. 
          ...
          so we decided to give the variable for the database in input and only when it is present the function access to it.
...
sub multiBlastout_2_gff3AndHash{
	...
	my $formatFile = shift;#Format (BLAST, BLASTXML for this one, actually it didn't work)
	my $gff3FileName = shift; 
	my $algorithmType = shift;	
	...
	my $uniprotDBType = shift;#SP or TR, only for these two database is used
  ...
  my $hitId;#Initialize the index of the HIT
    ...
    print "\nYou have $numOutFiles output files inside the folder $outputsDir...\n ";
		print "GFFPRODUCER - Algorithm: ".$algorithmType." - Query: ";
          ...
          $return = 1;
					print " (..Reading ($formatFile) from: $blastOut..) ";
              ...
              #The Name position change depending from the type of BLAST
                ...
                #print "seqID: $seqId\n";
								...
								if (defined $database){
									...
									$select_handle = $dbh->prepare_cached ("SELECT ".$os_taxTable.", description, go_ids FROM ".$view." WHERE seqId='".$seqId."'");
                  ...
                  if (scalar(@res) == 0){
                    print "WARNING: There is no result for the query (SELECT $os_taxTable, description, go_ids FROM $view WHERE "
                            ."seqId='$seqId') \n";
                    $discordantData++;
                  }
								}
                  ...
                  #and this control is also later in the GO and Descrs	 
                     ...
                     # print "Algorithm is blastx!\n";
                      ...
                      $outHash->{$qName}->{$uniprotDBType}->{"hitl"} = $hit->length;#Length of the HIT
											$outHash->{$qName}->{$uniprotDBType}->{"score"} = $hsp->evalue;
											$outHash->{$qName}->{$uniprotDBType}->{"strand"} = $hsp->strand('query');
                      ...
                      #print "name: ".$outHash->{$qName}->{$uniprotDBType}->{"name"}."\n";
									...
									if ($algorithmType eq "blastn"){
										...
										$outHash->{$qName}->{$algorithmType}->{"score"} = $hsp->evalue;
                    ...
                    $outHash->{$qName}->{$algorithmType}->{"strand"} = $hsp->strand('query');
                      ...
                      "hil=".$hit->length.";". #HIT LENGTH
                          ...
                          #hspId must be 2  because we take it only for the first result but
                          # at this point it has been summed
													if(($hspId == 2) and ($hitId == 1) and ($algorithmType eq "blastx")){
												...
												}else{print "WARNING: organism for $qName has not been found in the database. ".
                                "There's something wrong in the db writing or in the UNIPROT fasta file used as database. Check it!\n";}										
												if (defined $res[2]){
													my $goIds = $res[2];
													$goIds =~ s/;/ /g;
                          #chomp($goIds);
													print GFF_FILE ";go=".$goIds."\n";
													if(($hspId == 2) and ($hitId == 1) and ($algorithmType eq "blastx")){
														$outHash->{$qName}->{$uniprotDBType}->{"go"} = $goIds;
													} 
												}else{print GFF_FILE "\n";}
											}else { #Otherwise it takes the hit->description (BLASTN)
								...
								if (defined $database) {$select_handle->finish;}
                 $hitId++;
							}
              #We check here if there is discordant data between the blast output and data in database
              if ( $discordantData > 0){
                      print "Annocript has found that some information in file $blastOut are not in the database."
                              ." Two reasons are possible for this: \n\t1. you used a BLAST output with version different from that of database"
                              ." \n\t2. something in the header of database has been changed from uniprot.org. (In this case contact the producer of"
                              ." Annocript.\n";
              } 
...
sub blastnInfo{
    ...
    if(defined $outHash->{$id}->{'blastn'}->{'score'}) {
			push(@finalLine,$outHash->{$id}->{'blastn'}->{'score'});
    }else{push(@finalLine,"-");}
...
sub blastxInfosUniprot{
  ...
  my $putOS = shift;#it's a flag. If active we choose to insert the Organism name
  my $protDB = shift;
  ...
  if(defined $outHash->{$id}->{$uniprotDBType}){
    ...
    #hsp evalue
    if(defined $outHash->{$id}->{$uniprotDBType}->{'score'}){
      push(@finalLine, $outHash->{$id}->{$uniprotDBType}->{'score'});
    }else{push(@finalLine, "-");}
  ...
  #The organism is written only if OS_YES is set. Only for one result it will be written (SP or TR)
	if ($putOS eq "OS_YES"){
		#Printing organism 
		if(defined $outHash->{$id}->{$uniprotDBType}->{'os'}){
			 $os= "[".$uniprotDBType."]".$outHash->{$id}->{$uniprotDBType}->{'os'};
		}
	}
...
sub blastxInfosUnirefSP{
  ...
  #Enzyme and Pathways information only for Swiss-Prot
  if ( $uniprotDBType eq 'sp'){
    if(defined $outHash->{$id}->{$uniprotDBType}->{'name'}){
      push(@finalLine, enzymeInfo($outHash->{$id}->{$uniprotDBType}->{'name'},$protDB,$uniprotViewEnzyme));
      #$partDuration = time - $partTime;
      #print "Enzyme occurs ".$partDuration." seconds\n";
      #$partTime = time;
      push(@finalLine, pathwaysInfo($outHash->{$id}->{$uniprotDBType}->{'name'},$protDB,$uniprotWiewPathways));
      #$partDuration = time - $partTime;
      #print "Pathways occurs ".$partDuration." seconds\n";
      #$partTime = time;
    }else {push(@finalLine,"-","-","-");}
...
sub blastxGOInfos{
	...
	if(defined $outHash->{$id}->{$uniprotDBType}){
		 ...
		 if(defined $outHash->{$id}->{$uniprotDBType}->{'go'}){
			...
			#fetchs informations from the uniprot database
			foreach my $goTerm (@goTerms){
			 ...
			 if (scalar(@res) > 0){
					 ...
					 switch ($res[0]) {
						 case "P" { $bp=$bp.$res[2].$separator; $bpDef=$bpDef.$res[1].$separator}
						 case "F" { $mf=$mf.$res[2].$separator; $mfDef=$mfDef.$res[1].$separator}
						 case "C" { $cc=$cc.$res[2].$separator; $ccDef=$ccDef.$res[1].$separator}	
...
=head2 printAllInfo
 ...
 Function: This is the main subroutine to write the complete output file. It will flow the various hashes prepared with the
          ...
          The EnzymeIDs and Pathways are given only for Swiss-Prot.
...
sub printAllInfo{
 ...
 my $proteinDB = DBI->connect($dsn, $mySqlUser, $mySqlPass, { PrintError => 1, AutoCommit => 0 });
  ...
  my @ids4PrintInfo = collectFastaNames($configHash->{'fastaSeqs'});
 ...
 #Opening a file where print results
  ...
  print OUT_FILE $finalTabHeader."\n";
  print OUT_RID_FILE $ridFinalTabHeader."\n";
  ...
  my $countThresh = 1000;
    ...
    #my @elements = ();
		...
		my $sequence = $gffDB->fetch_sequence( -seq_id => $id );
    ...
    #Prints the number of sequences evaluated
		if ($counter % $countThresh == 0){
			print $counter." - ";
		}
		$counter++;
    
    #The first condition of the non codingness of a sequence is about its lenght
    if ( $seqLength >= $configHash->{'NCSeqLength'}){
      $NCflag++;
    }
    
    #my @countElements = ();
    my @countElements = countInfo($outHash,$id);
		#Print on a line the count
		print OUT_FILE join("\t",@countElements);
		print OUT_FILE "\t";
    
    print OUT_RID_FILE join("\t",@countElements);
		print OUT_RID_FILE "\t";
     ...
     if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){
      ...
      #my @blastxElements = ();
      my @blastxElements = separateBlastxTrSp($outHash,$id,$proteinDB,\$NCflag);
      print OUT_FILE join("\t",@blastxElements);
      print OUT_FILE "\t";
      #FOR THE PROGRAMMER:
      #The number of '-' when there is no results depends from the number of informations related to BLASTX/P and actually are 
      #24 using: SP and TR results, the enzyme, GO and Pathways columns. Please check this print everytime you will add a new column!!
      ...
      @blastxElements1 = blastxInfosUnirefSP($outHash,$id,$proteinDB,\$NCflag,'sp');
      @blastxElements2 = blastxInfosUnirefSP($outHash,$id,$proteinDB,\$NCflag,'uniref');
      #When the table is referred to Uniref database the GO term is associated with only Uniref and not SP.
      @blastxElements3 = blastxGOInfos($outHash,$id,'uniref',$proteinDB);      
      #FOR THE PROGRAMMER:
      #The number of '-' when there is no results depends from the number of informations related to BLASTX/P and actually are 
      #24 using: SP and TR results, the enzyme, GO and Pathways columns. Please check this print everytime you will add a new column!!
      
      #This includes the print of Enzyme and Pathways infos
      print OUT_FILE join("\t", @blastxElements1 );#
      print OUT_FILE "\t";
      print OUT_FILE join("\t", @blastxElements2 );#
      print OUT_FILE "\t";
      print OUT_FILE join("\t", @blastxElements3 );
      print OUT_FILE "\t";
      
      print OUT_RID_FILE join("\t", @blastxElements1 );#
      print OUT_RID_FILE "\t";
      print OUT_RID_FILE join("\t", @blastxElements2 );
      print OUT_RID_FILE "\t";
      print OUT_RID_FILE join("\t", @blastxElements3 );
      print OUT_RID_FILE "\t";
      #my $end = time();
      #printf(" blastx :%.2f\n", $end - $start);
    }
    ...
    $ncElement = nonCodingInfo($outHash,$id,\$NCflag);
		#Print on a line informations retrieved from the portrait output
		#print "Printing portrait informations...";
    ...
    #Prints our decision on if the seq is coding or not (1:non-coding;0:coding)
  ...
  if ( (defined $res[0])  ){
    ...
    foreach my $pwId (@pwIds){
      ...
      if (scalar(@res2) > 0){
        ...
        if ($descs eq "-"){
          $descs = $res2[0];
        }else{$descs = $descs.$separator.$res2[0];}
      }
...
sub separateBlastxTrSp{
  ...
  #SP and TR results, the enzyme, GO and Pathways columns. 
	my @emptyLine = ('-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-','-');	
      ...
      $$NCflag++;#SP
        ...
        $$NCflag++;#TR
          ...
          $$NCflag++;#SP
				...
				}elsif ($outHash->{$id}->{'tr'}->{'score'} < $outHash->{$id}->{'sp'}->{'score'}){
						$dbWinner = 'tr';
						}else{
						$dbWinner = 'sp';
						}
    ...
    #We put in the resulting table only the results that have an evalue lower than a given threshold
		if ($outHash->{$id}->{$dbWinner}->{'score'} <= $configHash->{'evalMax'} ){
			if ( $dbWinner eq 'sp'){
...
sub nonCodingInfo{
  ...
  my $NCflag = shift;#flag to use to find out if the sequence is non-coding
	...
	if( defined $outHash->{$id}->{'portrait'} ){
			#push(@finalLine,$outHash->{$id}->{'portrait'});
      $ncElement = $outHash->{$id}->{'portrait'};
      if ( $outHash->{$id}->{'portrait'} >= $configHash->{'NCThresh'}){
        $$NCflag++;#if there are no results the sequence could be non-coding
      }
  }#else {push($ncElement,"-");}
...
=head2 printORFFastaFile
 ...
 Function: Prints on a fasta file the longest orf with strand and frame both for 
          ...
          >Transcript_1 Swiss-Prot|O60488|1|2|Trembl|H2LVF5|1|2|DNA2PEP|721|+|2|
