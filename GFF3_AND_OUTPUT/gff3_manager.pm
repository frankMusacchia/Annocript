...
use USEFUL::utilities qw( extract_name print_array try_exec_command drop_database 
												destroy_db checkOutputsPresence delete_file);
...
sub execute_gff_manager{
 ...
 if ( $configHash->{'dbInUse'} eq 'uniref' ){
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE PORTRAIT OUTPUT<<<<<<<<<<<<<<\n";#LOG
    ...
    $partDuration = time - $partTime;
    print "Finished storing output from Portrait in $partDuration seconds\n";
