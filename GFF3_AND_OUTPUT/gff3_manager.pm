...
#Annocript libraries
use GFF3_AND_OUTPUT::output_and_stats	qw( lncRNA_coding_fasta_file_generator lncRNA_fasta_file_generator 
extractAnnotated extractStatistics );
use USEFUL::utilities qw( extract_name print_array try_exec_command drop_database destroy_db checkOutputsPresence);
...
sub execute_gff_manager{
  ...
  if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){
    ...
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxSPOut'},$configHash->{'blastxSPOutType'},$configHash->{'blastxSPGFF3FileName'},"blastx",1,$configHash->{'database'},'sp','OSType','os','uniprotkbViewSeq') > 0){
      ...
      $partDuration = time - $partTime;
      print "Finished GFF3 conversion of output with SwissProt in\t$partDuration\n";
      $partTime = time;
    ...
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxTROut'},$configHash->{'blastxTROutType'},$configHash->{'blastxTRGFF3FileName'},"blastx",1,$configHash->{'database'},'tr','OSType','os','uniprotkbViewSeq') > 0 ){
      ...
      if ( $configHash->{'useGFFDB'} eq 'YES' ){
        ...
        print "Finished storing GFF3 for blastx output from SwissProt in\t$partDuration\n";
        $partTime = time;  
      }
 ...
 if ( $configHash->{'dbInUse'} eq 'uniref' ){
   $os_tax='os';
    ...
    if ( multiBlastout_2_gff3AndHash($configHash->{'blastxSPOut'},$configHash->{'blastxSPOutType'},$configHash->{'blastxSPGFF3FileName'},"blastx",1,$configHash->{'database'},'sp','OSType','os','uniprotkbViewSeq') > 0){
      ...
      if ( $configHash->{'useGFFDB'} eq 'YES' ){
        gff3_2_db($configHash->{'blastxSPGFF3FileName'},$configHash->{'gffDB'},$configHash->{'mySqlUser'},$configHash->{'mySqlPass'});
        $partDuration = time - $partTime;
        print "Finished storing GFF3 for blastx output from Trembl in\t$partDuration\n";
        $partTime = time;
      }
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>EXTRACTION OF INFO FROM THE RPS BLAST OUTPUT<<<<<<<<<<<<<<\n";#LOG
    ...
    $configHash->{'rpstblastnGFF3FileName'},"rpstblastn",2,$configHash->{'database'},'none',undef,undef,'cdViewGO') > 0){
  ...
  printAllInfo($configHash->{'gffDB'},$configHash->{'outFileName'},$configHash->{'outFiltered'},$outHashFile);#$configHash->{'storeBlastx'},$outHashFile);
	$partDuration = time - $partTime;
	print "Finished printing the complete annotation in\t".$partDuration."\n";
	$partTime = time;
              ...
              #The Name position change depending from the type of BLAST
							...
							if (grep {$qName} @tr_ids){
								...
								while( my $hit = $result->next_hit ) {
									...
									#NEV CODE TO GET GO TERMS ASSOCIATED TO DOMAINS FROM THE VIEW
											...
											if ( $hsp->percent_identity >= 1 ) {
												...
												if (defined $database and (scalar(@res)>0)){
													...
													if (defined $res[2] and ($algorithmType eq "blastx")){
														...
														#We save in the hash only the go terms for the first result and only if the user chose to get it
														if(($hspId == 2) and ($hitId == 1) and ($algorithmType eq "blastx") and
													...
													if ((defined $res[0]) and ($algorithmType eq "rpstblastn") ){
														my $goIds = $res[0];
														$goIds =~ s/;/ /g;
														#print "Printing GO terms $goIds for domain\n";
														#chomp($goIds);
		 	...
		 	foreach my $domainGOs (@domainsGOs){
				...
				foreach my $goTerm (@goTerms){
					 ...
					 if (scalar(@res) > 0){
					   # print "Division: ".$res[0]." Def: ".$res[1]."\n";#DEBUG CODE
					  #if arguments are not null insert them in variables
						 if ($res[0] eq "P") { $bp=$bp.$res[2].$semicolon; $bpDef=$bpDef.$res[1].$semicolon}
						 if ($res[0] eq "F") { $mf=$mf.$res[2].$semicolon; $mfDef=$mfDef.$res[1].$semicolon}
						 if ($res[0] eq "C") { $cc=$cc.$res[2].$semicolon; $ccDef=$ccDef.$res[1].$semicolon}	
					 }#ENDIF 
														
				}#ENDFOREACH
					
				if (scalar(@res) > 0){
				  #changing all the separators at the end of the string
				  $bp =~ s/(\Q$semicolon\E)$/$separator/;
				  $mf =~ s/(\Q$semicolon\E)$/$separator/;
				  $cc =~ s/(\Q$semicolon\E)$/$separator/;
				  $bpDef =~ s/(\Q$semicolon\E)$/$separator/;
				  $mfDef =~ s/(\Q$semicolon\E)$/$separator/;
			 	 $ccDef =~ s/(\Q$semicolon\E)$/$separator/;
			 	}
...
sub printAllInfo{
 ...
 my $proteinDB = DBI->connect($dsn, $mySqlUser, $mySqlPass, { PrintError => 1, AutoCommit => 0 });
  ...
  for (my $counter = -1; $counter < scalar(@ids4PrintInfo); $counter++){
		...
		#Print the count info. Not available in the version for the publication
		...
		#########################
		#Print the best result (lowest evalue) among the HPSs found to be homologous 		
		if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){
			...
			if ($counter == -1){
			 ...
			 #my $start = time();
				...
				#x using: SP and TR results, the enzyme, GO and Pathways columns. Please check this print everytime you will add a new column!!
