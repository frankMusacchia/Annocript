...
#Annocript libraries
use USEFUL::utilities qw(dl_and_extract download_file check_presence print_array extract_name checkOutputsPresence);
...
sub setFieldNames {
  ...
  ## SEE ALSO HEADER IN GFF_MANAGER.pm
 ...
 if (open (ANNOUT_FILE,"<$annocriptOut")){
  ...
  while( $row = <ANNOUT_FILE> ) {
    ...
    if ( $fields[$headHash->{$NC4Annocript}] eq '1'){
      ...
      elsif ( ($fields[$headHash->{$Name1}] ne '-') or ($fields[$headHash->{$Name2}] ne '-') or ($fields[$headHash->{$CDName}] ne '-') or ( $fields[$headHash->{$longORFLength}] > $NCORFLength) ){
...
sub extractStatistics{
 ...
 #print "and now with the variable: ".$headHash->{$seq}."\n";
	...
	$configHash->{'portraitOutPresent'} = checkOutputsPresence($configHash->{'portraitOut'},$outFolder);
 ...
 my $existGOBPTermsDom = 0;
 my $existGOMFTermsDom = 0;
 my $existGOCCTermsDom = 0;
 my $existDomains = 0;
 my $existPwL1 = 0;
 my $existPwL2 = 0;
 my $existPwL3 = 0;
 ...
 for ($iRow = 0; $iRow < $totTranscripts; $iRow++ ){
  ...
  if ( ($configHash->{'goTermsAss'} eq 'domains') or ($configHash->{'goTermsAss'} eq 'both')){
	  ...
	  if($outAnnArray[$iRow][$headHash->{$domBPId}] ne "-"){  
	    $existGOBPTermsDom++;#If at least one result is present this will be increased
  ...
  if($outAnnArray[$iRow][$headHash->{$closerOS}] ne "-"){
     $existOrganisms++;
     my $organism = $outAnnArray[$iRow][$headHash->{$closerOS}] ;
     $nearOrg->{$organism}++;
  }
  ...
  if ( $outAnnArray[$iRow][$headHash->{$Strand2}] ne "-"){
     if ( $outAnnArray[$iRow][$headHash->{$Strand2}] eq "1" ){
		$strand2Pos++;
     }elsif( $outAnnArray[$iRow][$headHash->{$Strand1}] eq "-1" ){
		$strand2Neg++;
	 }
  }
 ...
 print OUT_STATS "The minimum and maximum sequences length are respectively $outAnnArray[$minSeqLengthInd][$headHash->{$transcrLength}]". 
 ...
 if ( ($configHash->{'goTermsAss'} eq 'domains') or ($configHash->{'goTermsAss'} eq 'both')){
	 #Print the go bp counts on a file 
	 $dataName = 'biological processes';
	 if ($existGOBPTermsDom>0){
	   $tableName = $configHash->{'bpStatsFileDom'};
	   printOutGOStats(\@headerPerc,\@headerExpr,$bpHashDom,$tableName,$totAnnotated,$statsFolder,\@outAnnArray,$dataName,$countsPresent); 
	   #print Dumper($bpHash);
	 }else{print "There are no $dataName classes in the output file. The statistics will not be created.\n";}
	 
	 #Print the go mf counts on a file
	  $dataName = 'molecular functions'; 
	 if ($existGOMFTermsDom>0){
	   $tableName = $configHash->{'mfStatsFileDom'};
	    printOutGOStats(\@headerPerc,\@headerExpr,$mfHashDom,$tableName,$totAnnotated,$statsFolder,\@outAnnArray,$dataName,$countsPresent); 
	    #print Dumper($mfHash);
	 }else{print "There are no $dataName classes in the output file. The statistics will not be created.\n";}
	 
	 #Print the go cc counts on a file
	 $dataName = 'cellular components';
	 if ($existGOCCTermsDom>0){
	   $tableName = $configHash->{'ccStatsFileDom'};
	   printOutGOStats(\@headerPerc,\@headerExpr,$ccHashDom,$tableName,$totAnnotated,$statsFolder,\@outAnnArray,$dataName,$countsPresent); 
	   #print Dumper($ccHash);
	 }else{print "There are no $dataName classes in the output file. The statistics will not be created.\n";}
 }
#Here we set an header to write in the pathways file table
 ...
 print OUT_STATS "Number of annotated sequences: ".$totAnnotated."\n\n";
  ...
  print OUT_STATS "Swiss-Prot results found with positive strand: $strand1Pos\n";
  print OUT_STATS "Swiss-Prot results found with negative strand: $strand1Neg\n";
  my $secondDB = 'UniRef';
  if ($configHash->{'dbInUse'} ne 'uniref'){
		$secondDB = 'TrEMBL';
  }
  print OUT_STATS "$secondDB results found with positive strand: $strand2Pos\n";
  print OUT_STATS "$secondDB results found with negative strand: $strand2Neg\n";
  ...
  if ( $configHash->{'dna2pepOutPresent'} eq 'YES'){
    print OUT_STATS "Sequences in agreement with strand of the longest ORF: ".detectLORFAgreement($orfFile,$outFolder)."\n";
  }
...
sub extractPWstatsFromCols {
  ...
  #Local variables  
  my @pwFound = ();#Elements in the row that have been already inserted
...
sub plots_2_html{
 ...
 my $R_seqs_stats_script = $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_seqs_stats_script'};
 ...
 foreach my $file (@files){
  if ($file =~ /$table/){
   ...
   #my $outImg = $file.".jpg";
   #PIE CHARTS ARE COMMENTED BUT IF YOU WANT YOU CAN PRINT ALSO CHANGING THE OUTPUT NAMES
   #my $command = "R CMD BATCH --no-save --no-restore '--args $file $outImg $name' tables_2_pie_chart.R";
   #BAR CHARTS ARE ACTIVE
   my $command = "R CMD BATCH --no-save --no-restore '--args $file $outImg $name $xlab $ylab $fontDim $topToShow' $R_plot_script $RLogPath";
 ...
 $configHash->{'cdStatsFile'}."|".$configHash->{'closerOSFile'}."|".$configHash->{'pathwaysL1File'}."|".$configHash->{'pathwaysL2File'}."|".
