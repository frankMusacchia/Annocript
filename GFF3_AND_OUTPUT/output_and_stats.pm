...
#Here we set an header to write in the pathways file table
 ...
 print OUT_STATS "Number of annotated sequences: ".$totAnnotated."\n\n";
  ...
  if ($strand1Pos > 0) {print OUT_STATS "Swiss-Prot results found with positive strand: $strand1Pos\n";}
  if ($strand1Neg > 0) {print OUT_STATS "Swiss-Prot results found with negative strand: $strand1Neg\n";}
  my $secondDB = 'UniRef';
  if ($configHash->{'dbInUse'} ne 'uniref'){
		$secondDB = 'TrEMBL';
  }
  if ($strand2Pos > 0) {print OUT_STATS "$secondDB results found with positive strand: $strand2Pos\n";}
  if ($strand2Neg > 0) {print OUT_STATS "$secondDB results found with negative strand: $strand2Neg\n";}
...
sub plots_2_html{
 ...
 if ($configHash->{'plotType'} eq 'piechart'){
		$R_plot_script = $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_piechart_script'};
 }else{
	 $R_plot_script = $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_barplot_script'};
 }
