...
=head1 AUTHORS
 ...
 $Id: Annocript,v 1.2 2015/22/04 11:00:12 Exp $
=cut
...
sub completeConfiguration{
  ...
  #annoPrint ("\n \n");
  checkVariable($configHash->{'doDbCreation'},'doDbCreation', "The database will be built downloading data from UniProt website.\n");
  ...
  if ($configHash->{'doExecutePrograms'} eq 'YES'){
    checkVariable($configHash->{'doBlastxSP'},'doBlastxSP',"A BLASTX against SwissProt will be executed.\n");
    checkVariable($configHash->{'doBlastxTRorUf'},'doBlastxTRorUf',"A BLASTX against TrEMBL/UniRef will be executed.\n");
  ...
  #Giving a path for the db versions file
    ...
    if (!check_DB_user_permissions($configHash->{'platform'},$configHash->{'host'},
      ...
      #print "Swiss-prot DB link...";
       ...
       if (!head($configHash->{'silvaLSULink'})) {
          if (!head($configHash->{'silvaLSULink'})) {
            if (testFTP($configHash->{'silvaLSULink'}) != 1) {
              annoPrint ("Please check if [".$configHash->{'silvaLSULink'}."] is a correct URL. Annocript will continue...\n"); 
              $wrongLinks++;
            }
          }
        }
        if (!head($configHash->{'silvaSSULink'})) {
          if (!head($configHash->{'silvaSSULink'})) {
            if (testFTP($configHash->{'silvaSSULink'}) != 1 ) {
                annoPrint ("Please check if [".$configHash->{'silvaSSULink'}."] is a correct URL. Annocript will continue...\n"); #
                $wrongLinks++;
            }
          }
        }
    ...
    if ( $configHash->{'doDbCreation'} ne "YES"){
        if( !(scalar <$mainDataFolder/*>) ){
          ...
          ( ($configHash->{'doBuildOutput'} eq "YES") or ($configHash->{'extractStatistics'} eq "YES") ) ){
						my $db_versions = $dataFolder."/".$configHash->{'db_versions_file'};
						print "Checking versions used previously in file $db_versions\n";
						#The database version is present in the version file
						open (VER_F,"<$db_versions")
							or die "Cannot open $db_versions. Maybe programs have not been executed before..\n";
						while (my $row = <VER_F>){
							if ( $row =~ /UniProt version/){
								chomp($row);
								$configHash->{'dbVersion'} = (split(":",$row))[1];
								annoPrint("Version of the database used previously is ".$configHash->{'dbVersion'}.".\n",1);
							}
							if ( $row =~ /UniProt database used/){
								chomp($row);
								$configHash->{'dbInUse'} = (split(":",$row))[1];
								annoPrint( "UniProt database used previously is ".$configHash->{'dbInUse'}.".\n");
							}
							if ( $row =~ /Domains database used/){
								chomp($row);
								$configHash->{'cdDB'} = (split(":",$row))[1];
								annoPrint( "Domains database used previously is ".$configHash->{'cdDB'}.".\n");
							}
							if ( $row =~ /UniRef type used/){
								chomp($row);
								$configHash->{'unirefDB'} = (split(":",$row))[1];
								annoPrint( "UniRef version used is ".$configHash->{'unirefDB'}.".\n");
							}
						}
						close(VER_F);
						...
						#Check if database chosen is present
						$configHash->{'database'} = $configHash->{'dbInUse'}.'_'.$configHash->{'dbVersion'};#Gives the name to the database
						annoDie("Annocript cannot find the mysql database".$configHash->{'database'}."\n") unless ( db_present($configHash->{'database'},$configHash->{'platform'},$configHash->{'host'},
						$configHash->{'port'},$configHash->{'mySqlUser'},$configHash->{'mySqlPass'}) > 0);
						annoPrint( "MySQL database in use is: ".$configHash->{'database'}."\n");
          ...
          ($configHash->{'doBuildOutput'} eq "NO") and ($configHash->{'extractStatistics'} eq "NO") ){
						annoDie("You are running nothing! Please use the user guide to learn how to use Annocript.\n");
					}
        ...
        #The user wants to execute the programs, therefore he can choose the database
          ...
          if ( ($useDb eq "y") or ($useDb eq 'Y') ) {
						...
						annoPrint( "Available database versions:\n");
						...
						$configHash->{'database'} = $configHash->{'dbInUse'}.'_'.$myFolder;	
						#The user chooses the CD database if it is present
						$configHash->{'cdDB'} = choose_CD_database($dbDataFolder);
          ...
          #The user has to change the config file	
			...
			$configHash->{'uniprotKBDB'} = "uniprot_kb_".$configHash->{'dbVersion'}.".fasta";#Creates the name for the uniprot kb	
			 ...
			 #Here I do a cross check. If the version of the database used does not corresponds
			 ...
			 check_db_tables_version($dsn,$configHash->{'mySqlUser'},$configHash->{'mySqlPass'}) 
				or annoDie( "The version of the database you selected is not compatible with this version of Annocript ($version) ". 
...
sub checkGFF3_AND_OUTPUT {
  ...
  if ( !($configHash->{'plotType'} =~ /(piechart?)|(barplot?)/)){
    annoDie( "You can choose only one between [piechart, barplot] as a value for plotType in the configuration file. ".$configHash->{'plotType'}." is not allowed!\n");
  } 
...
sub check_dependencies {
  ...
  if ($configHash->{'doExecutePrograms'} eq 'YES'){  
    ...
    if( (scalar <$outFolder/rpstblastn/*>) or (scalar <$outFolder/blastx/*>) or (scalar <$outFolder/blastn/*>) ) {
      my $removeOut = get_input("\n WARNING: Some output files already exist! If you will run a program (BLAST, PORTRAIT, DNA2PEP) the corresponding ".
                                "output will be overwritten. Instead, output of BLASTx against UniRef will be different from that against TrEMBL.".
  ...
  #are present
    ...
    if( !(scalar <$outFolder/rpstblastn/*>) and !(scalar <$outFolder/blastx/*>) and !(scalar <$outFolder/blastn/*>)) {
      annoDie("Output folders are empty. The output from Annocript cannot be obtained without that.".
...
sub check_Annocript_integrity {
  ...
  if ( ! ( (-e $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_seqs_stats_script'})
	   and (-e $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_piechart_script'})
	   and (-e $configHash->{'GFFAndOutputFolder'}."/".$configHash->{'R_barplot_script'})
  ...
  if ( $configHash->{'closerOSFile1'} !~ /^[A-za-z0-9\_\-\.]*$/i ){
		annoDie( "The name".$configHash->{'closerOSFile1'}." isn't permitted. Choose another in the config file using [A-za-z0-9\_\-]. Program will exit...\n");
	}
	if ( $configHash->{'closerOSFile2'} !~ /^[A-za-z0-9\_\-\.]*$/i ){
		annoDie( "The name".$configHash->{'closerOSFile2'}." isn't permitted. Choose another in the config file using [A-za-z0-9\_\-]. Program will exit...\n");
	}
  ...
  if ( $configHash->{'R_barplot_script'} !~ /^[A-za-z0-9\_\-\.]*$/i ){
		annoDie( "The name".$configHash->{'R_barplot_script'}." isn't permitted. Choose another in the config file using [A-za-z0-9\_\-]. Program will exit...\n");
	}  

  if ( $configHash->{'R_piechart_script'} !~ /^[A-za-z0-9\_\-\.]*$/i ){
		annoDie( "The name".$configHash->{'R_piechart_script'}." isn't permitted. Choose another in the config file using [A-za-z0-9\_\-]. Program will exit...\n");
	}  
  ...
  if ( $configHash->{'ncDB'} !~ /^[A-za-z0-9\_\-\.]*$/i ){
		annoDie( "The name".$configHash->{'ncDB'}." isn't permitted. Choose another in the config file using [A-za-z0-9\_\-]. Program will exit...\n");
	}
  ...
  if (@name > 1){
    ...
    if ( (length($configHash->{'name'}) > 50) # or !($configHash->{'fastaSeqs'} =~ /(\.fasta?)|(\.fa?)/) 
      ...
      if ( $newSession == 1 ){
        ...
        if (-e $fastaPath) {
          annoPrint ("Sequences found in your working folder. Checking...\n");
          #After the name all the fasta is checked to see if it respects the standards
					...
					if($warnings > 0){
						my $warningText = "Fasta check failed! The transcriptome you are using have the listed problems ".
													" which can cause stochastic error in BLAST programs execution. ".
													"Please try to correct the problems. Do you want to continue? [y|n] (Suggested: No) \n";
						my $sure = get_input($warningText,'^[YynN]$');
						annoPrint($sure."\n",1);
						if ( ( $sure eq "n") or ( $sure eq "N")){
							annoPrint("Ok, please correct the problem and re-start Annocript...\n");
							exit;
						}else{annoPrint("Ok, Annocript will use the fasta as it is. You will find possible errors in the log file.\n");}	
					}
          annoPrint ("Copying the fasta in session folder...\n");
        ...
        if (-e $sessionFolder.'/'.$configHash->{'fastaSeqs'}) {
          annoPrint ("Sequences found in your session folder. Checking it...\n");
          $configHash->{'fastaSeqsPresent'} = 'YES';
   				#After the name all the fasta is checked to see if it respects the standards
  ...
  foreach my $blastId (@blastIds){
    ...
    if (defined($configHash->{'min_port_seq_length'})){  
	  ...
	  if (defined($configHash->{'max_tentatives'})){ 
			if (!correct_type($configHash->{'max_tentatives'},"positiveint")){
				annoDie( $configHash->{'max_tentatives'}." is not a positive integer. Please change max_tentatives and re-execute Annocript. Exiting...\n");
			}  
		}
    ...
    if (defined($configHash->{'maxFastaSeqLen'})){  
			if (!correct_type($configHash->{'maxFastaSeqLen'},"positiveint")){
				annoDie( $configHash->{'maxFastaSeqLen'}." is not a positive integer. Please change maxFastaSeqLen and re-execute Annocript. Exiting...\n");
			}  
		}
...
sub parse_command_line_args{
  ...
  if (! (-e $ARGV[0]) ) {
		print "Config file ".$ARGV[0]." does not exists. Please write a correct path.\n";
    exit ;
  }
}
