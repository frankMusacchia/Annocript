...
package LIB::execute_programs;
use Exporter();

@ISA = qw(Exporter);
@EXPORT_OK = qw( execute_programs run_blast_indexing check_makeblastdb_out );
...
use LIB::files_management qw( delete_file delete_directory check_presence extract_name
								extract_special_chars append_str_2_file);

use LIB::annocript_management qw(indexed_db_present);
									
#Using a library to manage fasta files
use LIB::fasta_management qw( count_sequences create_slices );


use LIB::programs_management qw(exec_command log_and_exit print_and_log );
...
my @blastArgs = ('word_size','evalue','num_threads','num_descriptions','num_alignments','max_target_seqs','outfmt','threshold','matrix','threads');
...
sub execute_programs {
  ...
  $rpsblastPath = $configHash->{'rpsblastPath'};
	$lncRNAPredProgPath = $configHash->{'lncRNAPredProgPath'};
  ...
  my $programRuns = 0;
    ...
    if ($configHash->{'sequencesAlphabet'} eq 'dna'){
		...
		if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){
			if ($configHash->{'doBlastxSP'} eq "YES"){
				...
				&run_blast($fastaSeqs,$blastToUsePath,$configHash->{'swissProtDB'},$blastxSPOutPath);
      ...
      if ($configHash->{'doBlastxTRorUf'} eq "YES"){
				...
				&run_blast($fastaSeqs,$blastToUsePath, $configHash->{'tremblDB'},$blastxTROutPath);
    ...
    if ( $configHash->{'dbInUse'} eq 'uniref' ){
			...
			if ($configHash->{'doBlastxTRorUf'} eq "YES"){
				...
				&run_blast($fastaSeqs,$blastToUsePath,$configHash->{'unirefDB'},$blastxUnirefOutPath);
				#Check if the output is good
  ...
  if ($configHash->{'doBlastn'} eq "YES"){
    ...
    &run_blast($fastaSeqs,$blastToUsePath,$configHash->{'ncDB'}, $blastnOutPath);
    #Check the output if is good
  ...
  if ($configHash->{'sequencesAlphabet'} eq 'dna') {
    if ($configHash->{'doLNCPrediction'} eq "YES"){
      print "\n".scalar(localtime).": Running LNCPrediction...\n";#LOG
      #&portrait_split_and_run($fastaSeqs, $portraitPath,$configHash->{'portraitOut'});#Run portrait splitting the file in pieces
      &run_cpc2($fastaSeqs, $lncRNAPredProgPath,$configHash->{'lncRNAPredOut'});
      if (&checkOutput($configHash->{'lncRNAPredOut'},$totSequences,"lncRNAPredProg") == 0){
        print "WARNING! There had been some problems in the cpc2 running. The number of sequences".
            "in the output differs from the number of sequences in the query. Please check and try to restart Annocript\n";
      }
      $partDuration = time - $partTime;
      print "Finished CPC2 running in\t".$partDuration." seconds\n\n";
      $partTime = time;
      $programRuns++;
    }
  ...
  }else{ print "CPC2 and dna2pep will not be performed because you don't have DNA sequences...\n";}
  
  if ( $programRuns > 0 ) {
		$configHash->{'versions_file'} = $dataFolder."/".$configHash->{'versions_file'};
		#Remove if already present
		if ( -e $configHash->{'versions_file'} ){
				delete_file($configHash->{'versions_file'});
		}else {print "Cannot find ".$configHash->{'versions_file'}."\n" ;}
...
sub checkOutput{
  ...
  if (-e $fileOut){
    if (!(defined $configHash->{'outfmt'}) or ( $configHash->{'outfmt'} eq "0")){
      ...
      #In the case of CPC2 program the first line is the header
      if ($algorithm eq "lncRNAPredProg"){
        my $seqInOut = `grep -c '>' $fileOut`;#Count the number of queries done
        if ($seqInOut == ($numToCompare + 1)){
          $ok = 1;
						}
				}
...
=head2 run_diamond_indexing

 Title  : run_diamond_indexing
 Usage  : run_diamond_indexing( -file => 'query.fasta',
                -program => 'name of the program'
			    );

 Function: 	Starts diamond makedb that indicizes the fasta database that blast applications have to use

 Returns : the blast db indicized will be saved in the working directory

=cut
sub run_diamond_indexing {
	  my $file = shift;
  my $program = shift; 
  ...
  print "Creating index in DIAMOND mode...\n";
	...
	my $extensions = "diamondIndexedExts";
...
=head2 run_blast
 ...
 Usage  : run_blast( -file => 'the query file',
            ...
            -threads => 'number of thread to use for diamond'
 ...
 Returns : the diamond output will be saved in the working directory
...
=head2 run_diamond
 ...
 Title  : run_diamond
 Usage  : run_diamond( -file => 'the query file',
            -programPath => 'path to the blast program'
            -db => 'the db to use with blast'
...
sub run_diamond {
  ...
  if (defined($configHash->{'modeD'})){
		$query .= " --".$configHash->{'mode'}." ";
	} 
  $query = $query.' '.build_query_args('D').' --out '.$outFile;  
...
=head2 run_cpc2
 Title  : run_cpc2
 Usage  : run_cpc2( -file => 'the query file',
            -programPath => 'path to the program'
           -outFile => 'the name of the output file'
			    );

 Function: 	Executes CPC2 that gives a probability of be non coding to each of the sequences

 Returns : the CPC2 output will be saved in the working directory
...
sub run_cpc2 {
  ...
  #Since we can have problems with headers. We have to completely delete all the informations
  ...
  #Here we run cpc2 
  my $command = "python $programPath  -i $cleanFile -o $outFile";
  ...
  print "...DONE!\nCPC2 results written on $outFile\n";
}
