...
package LIB::db_creator;
use Exporter();

@ISA = qw(Exporter);
@EXPORT_OK = qw( execute_db_creator  );
...
#Annocript libraries
use LIB::execute_programs qw(  run_blast_indexing check_makeblastdb_out);


#Using a library to manage files
use LIB::files_management qw( delete_file delete_directory check_presence extract_name
								extract_special_chars append_str_2_file my_extract_any_file
								join_files_with_cat count_lines_file extract_columns_from_file
								append_str_2_file check_presence);

use LIB::annocript_management qw(indexed_db_present indexing_files_present getUnirefType
								my_get_proteoms_from_UniProt getSILVADBVersion );
									
#Using a library to manage fasta files
use LIB::fasta_management qw( count_sequences create_slices );


use LIB::programs_management qw(exec_command log_and_exit print_and_log try_exec_command);


use LIB::html_management qw(dl_and_extract download_file);

use LIB::std_lib qw(print_array deallocate_hash);	

use LIB::mysql_db_management qw(destroy_db);
...
#Variables to identify the database used. Till now they are for UNIPROT_kb and UNIREF If you want to add another please
...
sub execute_db_creator{
  ...
  download_databases($dbDownloaded, $dbDataFolder);
   ...
   #SINCE 2018 we are not downloading anymore SILVA db because we use RNACentral
  #containing both ncRNA and rRNA#  
...
sub download_databases {
  ...
  my $silvaLSULink;
  my $silvaSSULink;  
  my $ncRNADBLink = $configHash->{'ncRNADBLink'};
  ...
  #NCdatabases: rFam
  #Here we obtain the absolute name of the ncRNA database
  my $ncRNADBNameCom = extract_name($ncRNADBLink,"0");
  my $ncRNADBNameUnc = extract_name($ncRNADBLink,"gz");
  print "The file  $ncRNADBNameCom will be renamed with ". $configHash->{'ncDB'}."...\n";
  dl_and_extract($ncRNADBLink, $ncRNADBNameCom, $configHash->{'ncDB'}, $dbDataFolder);
  #For the ncRNA db use a standard name. Change it now
  move($dbDataFolder."/".$ncRNADBNameUnc,$configHash->{'ncDB'});
  ...
  #my $silva_ver = getSILVADBVersion($configHash->{'silvaVerLink'}, $dbDataFolder);#SILVA
  ##Substitute the version number to the string [VERSION] in the links
  ##I hope they will never add [VERSION] in their links :-D
  #$configHash->{'silvaLSULink'} =~ s/\[VERSION\]/$silva_ver/;#SILVA
  #$configHash->{'silvaSSULink'}=~ s/\[VERSION\]/$silva_ver/;#SILVA
  #$silvaLSULink = $configHash->{'silvaLSULink'};#SILVA
  #$silvaSSULink = $configHash->{'silvaSSULink'};#SILVA
  
  ##Here we obtain the absolute name of the Silva LSU database
  #my $silvaLSUNameCom = extract_name($silvaLSULink,"0");#SILVA
  #my $silvaLSUNameUnc = extract_name($silvaLSULink,"gz");#SILVA
  #dl_and_extract($silvaLSULink, $silvaLSUNameCom, $silvaLSUNameUnc, $dbDataFolder);#SILVA

  ##Here we obtain the absolute name of the Silva SSU database
  #my $silvaSSUNameCom = extract_name($silvaSSULink,"0");#SILVA
  #my $silvaSSUNameUnc = extract_name($silvaSSULink,"gz");#SILVA
  #dl_and_extract($silvaSSULink, $silvaSSUNameCom, $silvaSSUNameUnc, $dbDataFolder);#SILVA
  ...
  if ( $configHash->{'dbInUse'} eq 'uniprotkb' ){
    ...
    print "\n(".scalar(localtime).") >>>>>>>>>>>>>DOWNLOADING TREMBL AND SWISS-PROT FILES<<<<<<<<<<<<<<\n";#LOG
  ...
  if ( $configHash->{'dbInUse'} eq 'uniref' ){
    print "\n(".scalar(localtime).") >>>>>>>>>>>>>DOWNLOADING UNIREF AND SWISS-PROT FILES<<<<<<<<<<<<<<\n";#LOG
  ...
  print "\n(".scalar(localtime).") >>>>>>>>>>>>>Downloading dependencies for the mapping table<<<<<<<<<<<<<<\n";#LOG
...
=cut
 sub getRfamAndCdd {
  ...
  my $ncRNADBNameCom = extract_name($ncRNADBLink,"0");
  my $ncRNADBNameUnc = extract_name($ncRNADBLink,"gz");
 	my $riboDl = $dbDataFolder."/".$ncRNADBNameUnc; 
  ...
  if ($configHash->{'eraseFiles'} eq "YES"){
    ...
    delete_file($dbDataFolder."/".$ncRNADBNameCom);
    delete_file($riboDl);
...
sub createGOTable{
  ...
  if (not (-e $dbDataFolder."/".$GOtermsFile) ){
    #print "\n Downloading File: ".$GOTermsLink."\n";
    download_file($GOTermsLink,$dbDataFolder);
  }else{
    print "File ".$dbDataFolder."/".$GOtermsFile." already downloaded...\n";
  }
  ...
  while(my $row = <GOFILE> ) {
    ...
    if ($row =~ /^\[Term/){
      ...
      if ( $getflag == 1 ){
        ...
        if ($go_create == 1){
          print GOTABLE join("\t",$go_db_id, $go_id, $goName, $namespace."\n");
        } 
   		#print  join("\t",$go_db_id, $go_id, $goName, $namespace."\n");#DEBUGCODE
        
		#Put the db id into the hash
        $$goTableHash->{$go_id} = $go_db_id;
		$go_db_id++;
		
		if ( scalar(@go_alt_ids) > 0){
         foreach my $go_alt_id (@go_alt_ids){
           #print " - ".$key." - ";#DEBUGCODE
           if ($go_create == 1){
              print GOTABLE join("\t",$go_db_id, $go_alt_id, $goName, $namespace."\n");
           }
           $$goTableHash->{$go_alt_id} = $go_db_id;
           #print  join("\t",$go_db_id, $go_alt_id, $goName, $namespace."\n");#DEBUGCODE
           #print "\n";#DEBUGCODE
           $go_db_id++;
          }			
		}        
    ...
    if ($getflag == 1) {
		...
		if ($row =~ /^namespace:/){
			...
			if ( $namespace eq 'molecular_function'){
					$namespace = "F";
			}
			if ( $namespace eq 'cellular_component'){
					$namespace = "C";
			}			
		}
  ...
  #print "....DONE!(GOTABLE Upload completed)\n ";
}
  ...
  open(FILE,$file);
	...
	while (my $row = <FILE>) {
		 if ( ($row =~ /Release\sof/) and ($row =~ /^CC/) ){
			...
			append_str_2_file($configHash->{'db_versions_file'},$info_string);
		}
...
sub createMappingTable{
  ...
  createGOTable($configHash->{'GOTermsLink'},\$goTableHash,$unique_creation);
  #print "\n....DONE!\n";
  $partDuration = time-$partTime;
