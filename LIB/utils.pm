...
use LIB::files_management qw( extract_name download_file );
...
sub check_blastx_status {
 ...
 if( (scalar <$jobs/*>) ){
  ...
  if ($typeIn eq "t"){ $type = "TR";}
  if ($typeIn eq "s"){ $type = "SP";}
  if ($typeIn eq "u"){ $type = "Uniref";}
...
sub check_portrait_status {
 ...
 my $fileName = 'lncRNAPredOut'; 

 if( (scalar <$jobs/*>) ){
  print "Please insert the name of the folder you want to inspect CPC2 status\n";
  my $workDir = getcwd;
  chdir $jobs;#go to jobs directory
  ...
  $myFolder = $jobs."/".$myFolder;
  #Now entering in the rps out folder and checking if it is empty
  my $lncRNAPredOut = $myFolder.'/output/'.$fileName;
  my $done;
  if ( (-e $lncRNAPredOut) ){
		my $command = "wc -l $lncRNAPredOut";
    try_exec_command($command) > 0 or die "Error: unable to execute $command\n";
  }else{
    print "No file $lncRNAPredOut has been found..\n ";
  }
 }else{
    die "The folder $jobs is empty\n";
 } 
